...
void usage(char *name)
{
	...
	fprintf(stderr, "     \t of the enabled pins (use test1 program to configure them).\n");
    fprintf(stderr, "\n");
	exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
	...
	/*
	 * Use test1 program to enable IRQ generation and enabling triggers
	...
	res = read(fd, NULL, 0);
	if (res < 0) {
		fprintf(stderr, "Err, read failed [%d]\n", res);
		exit(EXIT_FAILURE);
	}
